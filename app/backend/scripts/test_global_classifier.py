...
cir_themes = canonical_170["Cirurgia"]
cli_themes = canonical_170["Clínica Médica"]
go_themes = canonical_170["Ginecologia e Obstetrícia"]
ped_themes = canonical_170["Pediatria"]
prev_themes = canonical_170["Medicina Preventiva"]

questions = conn.execute("SELECT id, area, subtema, topic, stem FROM questions").fetchall()
print(f"Total questions to classify: {len(questions)}")

# =========================================================================
# GLOBAL SEMANTIC CLASSIFIER RULES
# =========================================================================
...
def classify_question(q):
    ...
    if "Pediatria" in raw_area or "criança" in text[:50] or "lactente" in text[:50] or "recém-nascido" in text[:50] or "pediatria" in topic.lower():
    ...
    elif "Cirurgia" in raw_area or "cirurgia" in topic.lower() or "trauma" in topic.lower():
        ...
        # Fallback to keyword search for null/unknown areas
    ...
    if target_area == "Cirurgia":
        ...
        if any(re.search(p, text) for p in [r"trauma raquimedular", r"\btrm\b", r"choque neurogênico", r"choque medular", r"fratura de coluna"]):
        ...
        if any(re.search(p, text) for p in [r"fratura exposta", r"síndrome compartimental", r"trauma de extremidade"]):
        ...
        # Ortopedia
        if any(re.search(p, text) for p in [r"fratura", r"consolidação óssea", r"osteossíntese", r"rádio distal", r"colo do fêmur"]):
            return ("Cirurgia", "Fraturas Ósseas")
        if any(re.search(p, text) for p in [r"luxação", r"entorse", r"ligamento cruzado", r"menisco", r"lesão ligamentar"]):
        ...
        if any(re.search(p, text) for p in [r"osteossarcoma", r"tumor ósseo", r"sarcoma ósseo"]):
        ...
        if any(re.search(p, text) for p in [r"câncer de tireoide", r"tumor de parótida", r"esvaziamento cervical"]):
    ...
    if target_area == "Clínica Médica":
        ...
        if any(re.search(p, text) for p in [r"fibrose pulmonar idiopática", r"pneumonite por hipersensibilidade", r"sarcoidose", r"doença pulmonar intersticial"]):
        ...
        if any(re.search(p, text) for p in [r"síndrome de guillain-barré", r"miastenia gravis", r"esclerose múltipla", r"epilepsia no adulto"]):
        ...
        if any(re.search(p, text) for p in [r"vasculite anca", r"granulomatose de wegener", r"panarterite nodosa", r"churg-strauss", r"takayasu"]):
        ...
        if any(re.search(p, text) for p in [r"intoxicação por paracetamol", r"intoxicação por opioides", r"acidente botrópico", r"acidente crotálico", r"escorpionismo"]):
...
for q in questions:
    ...
    results.append((target_area, target_sub, q["id"]))
    area_counts[target_area] = area_counts.get(target_area, 0) + 1
    sub_counts[(target_area, target_sub)] = sub_counts.get((target_area, target_sub), 0) + 1

print("\n=== CLASSIFICATION SUMMARY ACROSS 5 AREAS ===")
for a, c in area_counts.items():
    print(f" - {a}: {c} questions")

print(f"\nTotal questions classified: {len(results)}")
print(f"Total distinct (Area, Subtema) populated: {len(sub_counts)} / 170")
