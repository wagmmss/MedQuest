...
for area_group in planner_data:
    ...
    for m in macros:
        ...
        # Check if this macro theme has any questions
...
with open("zero_questions_report.json", "w", encoding="utf-8") as f:
    json.dump(zero_themes_by_area, f, ensure_ascii=False, indent=2)

for area, zeros in zero_themes_by_area.items():
    print(f"\nArea: {area} ({len(zeros)} temas com 0 questões):")
