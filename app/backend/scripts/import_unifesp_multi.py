"""
Script para importação e substituição completa de provas em lote do arquivo UNIFESP_2020-2026.har.
Processa individualmente cada um dos 7 anos (2020 a 2026), garantindo 100 questões por ano,
100% no padrão Template Ouro, com imagens preservadas e lote autoral protegido.
"""
...
INST_CODE = "UNIFESP"
INST_LABEL = "UNIFESP - Hospital Universitário da UNIFESP"
...
def import_unifesp_multi(har_path: str):
    print(f"[HAR] Lendo arquivo: {har_path}")
    with open(har_path, "r", encoding="utf-8", errors="ignore") as f:
        har = json.load(f)

    # 1. Agrupar questões por teste
    ...
    for (tid, tname), raw_qs in sorted(test_map.items(), key=lambda x: x[0][1]):
        # Extrair ano do teste
        year_m = re.search(r"\b(202\d)\b", tname)
        if not year_m:
            continue
        year = int(year_m.group(1))

        # Deduplicar mantendo a ordem sequencial original
        seen = set()
        uniq_qs = []
        for q in raw_qs:
            ...
            if qid not in seen:
                seen.add(qid)
                # Filtrar discursivas se houver
                sku = q.get("sku", "")
                if "DISCURSIVA" not in sku and "TEEM" not in sku:
                    uniq_qs.append(q)

        print(f"\n==================================================")
        print(f"[PROCESSANDO] {tname} -> Ano {year} ({len(uniq_qs)} questões)")
        print(f"==================================================")

        # 1. Remover questões antigas não-autorais deste ano
        cursor.execute("""
        ...
        print(f"[SUCESSO] Inseridas {len(uniq_qs)} questões oficiais para UNIFESP {year}!")

    conn.commit()
    conn.close()

    print(f"\n==================================================")
    print(f"[FINALIZADO] Total de questões UNIFESP inseridas: {total_inserted}")
    print(f"[FINALIZADO] Total de imagens indexadas: {total_images}")
    print(f"==================================================")

if __name__ == "__main__":
    har_path = r"C:\Users\wmors\Downloads\UNIFESP_2020-2026.har"
    import_unifesp_multi(har_path)
