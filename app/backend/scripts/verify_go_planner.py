...
for w in plan:
    for t in w.get("topics", []):
        if "Ginecologia" in t.get("area", "") or "Obstetr" in t.get("area", ""):
            go_topics.append(t)

print(f"\nTotal Ginecologia e Obstetrícia topics in plan: {len(go_topics)}")
for i, t in enumerate(go_topics):
...
assert len(go_topics) == 37, f"Expected 37 GO topics in plan, got {len(go_topics)}"
print("\n>>> ALL 37 GINECOLOGIA E OBSTETRÍCIA MODULES VERIFIED PERFECTLY IN PLAN! <<<")
