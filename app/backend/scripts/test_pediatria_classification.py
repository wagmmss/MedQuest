...
print(f"Total Pediatria questions: {len(questions)}")

def classify_ped_question(subtema, topic, stem):
    text = (str(topic) + " " + str(stem)).lower()
    
    # 1. Neonatologia - Sala de parto e Triagem
    if any(re.search(p, text) for p in [r"sala de parto", r"reanimação neonatal", r"clampeamento do cordão", r"apgar", r"vpp"]):
    ...
    if any(re.search(p, text) for p in [r"sífilis congênita", r"toxoplasmose congênita", r"citomegalovírus congênito", r"zika congênita", r"sepse neonatal", r"infecção congênita", r"storch"]):
        return "Período Neonatal: Doenças Infecciosas"
    if any(re.search(p, text) for p in [r"doença da membrana hialina", r"síndrome do desconforto respiratório", r"taquipneia transitória", r"ttrn", r"síndrome de aspiração meconial", r"sam"]):
    ...
    if any(re.search(p, text) for p in [r"marcos do desenvolvimento", r"desenvolvimento neuropsicomotor", r"dnpm", r"escala de denver", r"reflexos primitivos", r"crescimento", r"curva de crescimento", r"escore z", r"percentil"]):
    ...
    if any(re.search(p, text) for p in [r"vacina", r"imunizaç", r"pni", r"bcg", r"pentavalente", r"poliomielite", r"vip", r"vop", r"pneumocócica", r"meningocócica", r"tríplice viral", r"varicela", r"febre amarela", r"rotavírus"]):
        return "Imunizações"
    if any(re.search(p, text) for p in [r"sarampo", r"rubéola", r"exantema súbito", r"roséola", r"eritema infeccioso", r"parvovírus", r"varicela", r"catapora", r"mão-pé-boca", r"escarlatina", r"kawasaki", r"doença de kawasaki"]):
    ...
    # 6. Pneumologia e ORL
    if any(re.search(p, text) for p in [r"asma", r"bronquiolite", r"vírus sincicial", r"vsr", r"laringite", r"crupe", r"estridor", r"corpo estranho em via aérea"]):
    ...
    # 7. Gastro
    if any(re.search(p, text) for p in [r"diarreia aguda", r"gastroenterite", r"desidratação", r"tro", r"plano a", r"plano b", r"plano c", r"doença celíaca", r"alergia à proteína do leite", r"\baplv\b"]):
    ...
    if any(re.search(p, text) for p in [r"síndrome nefrítica", r"gnpe", r"síndrome nefrótica", r"hematúria", r"proteinúria"]):
        return "Desordens Genéticas e Erros Inatos do Metabolismo" # or Nefro

    # 9. Cardiologia
    if any(re.search(p, text) for p in [r"cardiopatia congênita", r"cia", r"civ", r"pca", r"tetralogia de fallot", r"coarctação de aorta", r"transposição das grandes artérias"]):
...
counts = {}
for q in questions:
    target = classify_ped_question(q["subtema"], q["topic"], q["stem"])
    counts[target] = counts.get(target, 0) + 1

print("\nDistribution across 28 Medway Pediatria Modules:")
