...
for w in plan:
    for t in w.get("topics", []):
        if t.get("area") == "Preventiva":
            prev_topics.append(t)

print(f"\nTotal Preventiva topics in plan: {len(prev_topics)}")
for i, t in enumerate(prev_topics):
...
assert len(prev_topics) == 13, f"Expected 13 Preventiva topics in plan, got {len(prev_topics)}"
print("\n>>> ALL 13 MEDICINA PREVENTIVA MODULES VERIFIED PERFECTLY IN PLAN! <<<")
