...
for (tid, tname), qs in test_map.items():
    # Deduplicate within test
    seen = set()
    uniq_qs = []
    for q in qs:
        qid = q.get("questionIdentifier") or q.get("_id")
    ...
    for q in uniq_qs[:3]:
        print(f"   - SKU: {q.get('sku')!r} | Ano={q.get('year')} | Gabarito: {[chr(65+i) for i, a in enumerate(q.get('answers', [])) if a.get('rightAnswer')]}")
