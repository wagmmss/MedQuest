...
def overview():
    ...
    srs_due_count = db.execute(
        "SELECT COUNT(*) n FROM spaced_repetition WHERE next_review_date <= ?", (now_utc.isoformat(),)
    ).fetchone()["n"]
...
def recommendations():
    ...
    area_answered = db.execute("""
        ...
        FROM attempts a JOIN questions q ON q.id = a.question_id
        WHERE q.area IS NOT NULL AND q.area != '' GROUP BY q.area
    """).fetchall()
...
def distractors():
    ...
    rows = db.execute("""
        ...
        FROM attempts a JOIN questions q ON q.id = a.question_id
        WHERE a.is_correct = 0 AND q.subtema IS NOT NULL AND q.subtema != ''
