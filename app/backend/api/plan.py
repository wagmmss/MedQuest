...
def planner_config():
    ...
    if request.method == "POST":
        ...
        """, (cfg.exam_date, cfg.start_date, cfg.days_per_week, cfg.questions_per_day,
    ...
    return jsonify({
        ...
        "days_per_week": row["days_per_week"], "questions_per_day": row["questions_per_day"],
    })
...
def planner_study(week):
    ...
    if studied:
        db.execute("""
            INSERT INTO planner_progress (week, studied, studied_at) VALUES (?, 1, ?)
...
def planner_revision(week):
    ...
    db.execute(f"""
        INSERT INTO planner_progress (week, {data.type}) VALUES (?, ?)
        ON CONFLICT(week) DO UPDATE SET {data.type} = ?
...
def generate_plan():
    ...
    plan = generate_annual_plan(DB_PATH, start_date, data.exam_date, data.hours_per_week)
