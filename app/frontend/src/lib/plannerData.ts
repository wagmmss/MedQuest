...
export function getSubtemaDetails(subtema: string) {
  for (const group of plannerData) {
    for (const macro of group.macroThemes) {
      if (macro.dbSubtemas?.includes(subtema)) {
        return {
          macroThemeName: macro.theme,
          highYield: macro.highYield,
