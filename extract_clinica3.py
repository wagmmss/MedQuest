...
print(f"Found {len(clinica_theme_names)} themes in Clínica Médica from /modules/ endpoint.")
            ...
            try:
                ...
                for item in body:
                    if item.get('discipline_name') == 'Clínica Médica':
                        foco_themes.add(item.get('theme_name'))
            except: pass

print(f"Found {len(foco_themes)} high yield themes (FOCO) for Clínica Médica.")
...
# Handle "Como cai" and map High Yield
final_themes = {}
for name, data in clinica_themes.items():
    ...
    final_themes[base_name]['practice_hours'] += data['practice_hours']
    final_themes[base_name]['total_hours'] += data['total_hours']

# Since 'theme_name' from focus might map to 'base_name', let's check
for theme in foco_themes:
    # the theme in focus API might exactly match or be slightly different
    # e.g. "Cardiologia", "Pneumologia". Wait, let's see what foco_themes are!
    pass

with open('clinica_extracted_focos.json', 'w', encoding='utf-8') as f:
    json.dump(list(foco_themes), f, indent=2, ensure_ascii=False)

with open('clinica_extracted_themes.json', 'w', encoding='utf-8') as f:
    json.dump(final_themes, f, indent=2, ensure_ascii=False)

print(f"Processed {len(final_themes)} final themes for Clínica Médica.")
